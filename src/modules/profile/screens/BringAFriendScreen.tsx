...
const mockReferral: ReferralInfo = {
  code: 'RAIAN10',
  url: 'https://studiowellness.app/ref/RAIAN10',
  totalReferrals: 3,
...
const BringAFriendScreen = ({ navigation }: any) => {
  ...
  const handleShare = async () => {
    try {
      await Share.share({
        message: `Vem treinar comigo no Studio Wellness! Use meu código ${referral.code} e ganhe créditos extras. Baixe o app: ${referral.url}`,
        title: 'Studio Wellness - Bring a Friend',
      });
    } catch {
  ...
  const handleCopy = () => {
    showAlert({ title: 'Copiado!', message: 'Código copiado para a área de transferência' });
  };
