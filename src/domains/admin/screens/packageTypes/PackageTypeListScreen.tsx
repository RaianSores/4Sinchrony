...
const PackageTypeListScreen = ({ navigation }: any) => {
  ...
  const handleToggleActive = async (item: AdminPackageType, value: boolean) => {
    setTogglingId(item.id);
    try {
      await packageTypeAdminService.update(item.id, { ...item, active: value } as any);
      setTypes(prev => prev.map(x => (x.id === item.id ? { ...x, active: value } : x)));
    } catch (error) {
      captureError(error);
    } finally {
      setTogglingId(null);
