...
const ClassDetailScreen = ({ navigation, route }: any) => {
  ...
  const handleBook = async () => {
    ...
    if (!hasCredits) {
      showAlert({
        title: 'Sem créditos',
        message: 'Você não possui créditos disponíveis. Adquira um plano para reservar aulas.',
        buttons: [
          { text: 'Fechar', style: 'cancel' },
          { text: 'Ver Planos', onPress: () => navigation.navigate('ProfileTab', { screen: 'Packages' }) },
        ],
      });
      return;
    }
...
const styles = StyleSheet.create({
  ...
  creditsText: { color: theme.colors.textSecondary, fontSize: 14, fontWeight: '500' },
  bookedInfo: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
